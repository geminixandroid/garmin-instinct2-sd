...
class GarminSDComms {
  ...
  function onSdStatusReceive(responseCode as Number, data as Null or Dictionary or String or Toybox.PersistedContent.Iterator) as Void {
    ...
    if (responseCode == 200) {
      var statusDict = data as Dictionary;
      if (responseCode != lastOnSdStatusReceiveResponse) {
        needs_update = true;
        // writeLog(tagStr, "needs update 1");
        writeLog(tagStr, "Status =" + statusDict.get("alarmPhrase"));
      }
      if (!mAccelHandler.mStatusStr.equals(statusDict.get("alarmPhrase"))){
        mAccelHandler.mStatusStr = (statusDict.get("alarmPhrase") as String);

        writeLog(tagStr, statusDict.get("alarmPhrase"));
        needs_update = true;
      }
      if (statusDict.get("alarmState") != 0) {
        try {
  ...
  function onDataReceive(responseCode as Number, data as Null or Dictionary or String or Toybox.PersistedContent.Iterator) as Void  {
    ...
    //writeLog(tagStr, "sendAccelData End - Send Duration = " + Time.now().subtract(mDataSendStartTime).value());
    if (responseCode == 200) {
      mAccelHandler.mStatusStr = "---";
